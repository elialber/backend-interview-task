...
describe('Edit Account E2E', () => {
  ...
  it('should allow an admin to update name and role', async () => {
    ...
    expect(mockUserRepository.save).toHaveBeenCalledWith(expect.objectContaining({ name: 'New Name', role: 'admin' }));
  });
  ...
  it('should allow a user to update their name and set isOnboarded to true', async () => {
    ...
    // Assert
    expect(response.status).toBe(200);
    expect(mockUserRepository.save).toHaveBeenCalledWith(expect.objectContaining({ name: 'New Name', isOnboarded: true }));
  });
  ...
  it('should not allow a user to update their role', async () => {
    ...
    expect(response.body).toEqual({ message: 'Only admins can change the role' });
  ...
  it('should return 404 if user is not found', async () => {
    // Arrange
    (authMiddleware as jest.Mock).mockImplementation(async (ctx, next) => {
      ctx.state.user = { email: 'notfound@example.com', 'cognito:groups': ['user'] };
      await next();
    });
